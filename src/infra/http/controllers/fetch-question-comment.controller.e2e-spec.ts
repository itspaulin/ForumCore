...
describe('Fetch questions comments (E2E)', () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [StudentFactory, QuestionsFactory, QuestionCommentFactory],
    }).compile()
  ...
  test('[GET] /questions/:questionId/comments', async () => {
    ...
    await Promise.all([
      questionCommentFactory.makePrismaQuestionsComment({
        authorId: user.id,
    ...
    expect(response.body).toEqual({
      comments: expect.arrayContaining([
        expect.objectContaining({ content: 'comment 01', authorName: 'John' }),
        expect.objectContaining({ content: 'comment 02', authorName: 'John' }),
      ]),
    })
  })
})
